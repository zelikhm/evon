    ...
    <div class="grid grid-cols-5 xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-5 xxl:gap-4 xl:gap-3">
      ...
      <div class="flex flex-col" v-for="item in slider">
        <Link class="object__block relative">
          <img :src="'/storage/' + item.image" class="w-full h-full rounded-[8px]" alt="">
          <div class="flex items-center gap-2 absolute bottom-0 left-0 p-5 xxl:p-4 xl:p-3 text-white text-[14px] xxl:text-[12px] xl:text-[10px]">
        ...
        <div class="flex flex-col p-5 xxl:p-4 xl:p-3 gap-4 xxl:gap-3 xl:gap-2.5">
          <span class="text-[#1E1D2D] font-semibold text-[18px] xxl:text-[15px] xl:text-[13px] leading-none">{{ item.title }}</span>
          <span class="text-[#1E1D2D] text-[18px] xxl:text-[15px] xl:text-[13px] leading-none">от {{ Number.isInteger(item.minPrice) ? item.minPrice : "-" }} ₽</span>
          <div class="text-[#8A8996] text-[14px] xxl:text-[12px] xl:text-[10px] flex items-center gap-2">
