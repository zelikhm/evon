          ...
          <div class="flex md:flex-col md:items-center h-20 xxl:h-16 xl:h-12 md:h-fit">
            <div class="border__right md:border-0 flex items-center md:flex-col gap-5 xxl:gap-4 xl:gap-3 p-4 xxl:p-3 xl:p-2">
              ...
              <div @click="map = !map" class="button__map text-[#6435A5]  flex items-center justify-center cursor-pointer bg-[#F6F3FA] gap-2 xxl:gap-1.5 xl:gap-1 h-10 xxl:h-8 xl:h-6 lg:h-8 px-4 xxl:px-3 xl:px-2.5 rounded-[4px]">
                ...
                <span class="text-sm xxl:text-xs xl:text-[10px] lg:text-[15px] leading-none">{{ map ? "Скрыть карту" : "Показать на карте" }}</span>
              </div>
            </div>
      ...
      <div class="grid__68-30 justify-between" v-if="!map">
        <div class="w-full flex flex-col">
          ...
          <div class="flex flex-col pb-14 xxl:pb-10 xl:pb-8">
            ...
            <p class="text-[#8A8996] text-base xxl:text-sm xl:text-xs pb-6 xxl:pb-5 xl:pb-4" v-if="fullDescription && house.description.length < 1000">{{ house.description }}</p>
            <p class="text-[#8A8996] text-base xxl:text-sm xl:text-xs pb-6 xxl:pb-5 xl:pb-4" v-else>{{ house.description.slice(0, 1000) + '...' }}</p>
            <button class="flex gap-2 xxl:gap-1.5 xl:gap-1 items-center w-fit animation__arrow" @click="fullDescription = !fullDescription" v-if="house.description.length > 1000">
