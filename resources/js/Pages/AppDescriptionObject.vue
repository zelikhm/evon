<template>
  <app-header :user="user" @update-block-client="updateBlockClient" :language="language"
    @selectLanguage="choseLanguage" />
  <app-modal-album v-if="album" :image="house.images" @close-album="album = false" :language="language" />
  ...
  <app-add-selections v-if="openAddSelection" @open-i-create-selections="openCreateSel"
    ...
    :area_array="area_array" :house="house" :language="language" />
  <app-immovables-create-selection v-if="openCreateSelection" @close-i-create-selection="closeCreateSel"
    @updateCompilation="updateCompilation" :house="house" :user="user" :language="language" />
  <app-all-news v-if="openAllNews" @close-all-news="openAllNews = false" :language="language" :house="house" />
  <app-chess v-if="chess" :house="house" @close-chess="closeChess" @open-cheme="openScheme" :language="language" />
  <app-chess-scheme v-if="openCheme" :flat="selectFlat" @closeChess="openCheme = false" :language="language" />
  <main v-if="!chess">
      ...
      <div class="decription__head h-20 xxl:h-16 xl:h-12 lg:h-fit rounded-[12px] my-7 xxl:my-5 xl:my-4">
        <div class="flex lg:flex-col items-center justify-between">
          ...
          <div class="flex flex-col p-4 xxl:p-3 xl:p-2">
            ...
            <span class="text-[#8A8996] text-center text-sm xxl:text-xs xl:text-[10px] lg:text-[14px]">{{ selectLanguage
              === 0 ? city_name.title : selectLanguage === 1 ? city_name.title_en : city_name.title_tr }}, {{ area_name
    !== undefined ? selectLanguage === 0 ? area_name.title : selectLanguage === 1 ? area_name.title_en :
      area_name.title_tr : '' }}</span>
          </div>
          ...
          <div class="flex lg:flex-col lg:items-center h-20 xxl:h-16 xl:h-12 lg:h-fit">
            <div
              ...
              <div @click="map = !map"
                ...
                <svg width="24" height="24" class="h-6 xxl:h-5 xl:h-4 lg:h-5" viewBox="0 0 24 24" fill="none"
                ...
                <span class="text-sm xxl:text-xs xl:text-[10px] lg:text-[15px] leading-none">{{ map ?
                  language.menu_zastr_1[11] : language.rielt_1[6] }}</span>
              </div>
            </div>

            <div v-if="user.role === 0"
              class="border__right lg:border-0 flex items-center gap-3 xxl:gap-2 xl:gap-1.5 p-4 xxl:p-3 xl:p-2">
              <button @click="openAddSelection = true"
                ...
                <div
                  ...
                  {{ language.rielt_1[14] }}</div>
              </button>
              <button v-if="!flagFavorite" @click="addToFavorite"
                ...
                <div
                  class="seek absolute opacity-0 overflow-hidden pointer-events-none px-3 xxl:px-2.5 xl:px-2 bg-white border border-solid border-[#E5DFEE] rounded-[3px] z-10 text-base xxl:text-sm xl:text-xs top-[140%] -left-1/2 whitespace-nowrap transition-all">
                  {{ language.rielt_1[16] }}</div>
              </button>
              <button v-if="flagFavorite" @click="removeToFavorite"
                class="relative heart__hover flex items-center justify-center h-10 xxl:h-8 xl:h-6 lg:h-8 w-10 xxl:w-8 xl:w-6 lg:w-8 bg-[#F6F3FA] rounded-[3px]">
      ...
      <app-map @open-add-selections="openAddSelection = true" v-if="map" :houses_array="markers" :city="city"
        :houses="house" :type="3" :state="false" />
      <div class="grid__68-30 justify-between" v-if="!map">
        <div class="w-full flex flex-col">
          <swiper :navigation="true" :modules="modules" :loop="true" class="mySwiper w-full">
          ...
          <div
            ...
            <div class="flex flex-col justify-center sm:p-2">
              <span class="text-[18px] xxl:text-[15px] xl:text-[13px] lg:text-[16px] text-center leading-none">{{
                house.info.type === 'Новостройка' ? language.rielt_1[0] : language.rielt_1[1] }}</span>
              <span class="text-center text-[#8A8996] text-[13px] xxl:text-[11px] xl:text-[9px] lg:text-[13px]">{{
            ...
            <div class="flex flex-col justify-center sm:p-2">
              <span class="text-[18px] xxl:text-[15px] xl:text-[13px] lg:text-[16px] text-center leading-none">{{
                Number.isInteger(minPriceForM) ? minPriceForM.toLocaleString('ru') : "-" }} €</span>
              <span class="text-center text-[#8A8996] text-[13px] xxl:text-[11px] xl:text-[9px] lg:text-[13px]">{{
            ...
            <div class="flex flex-col justify-center sm:p-2">
              <span class="text-[18px] xxl:text-[15px] xl:text-[13px] lg:text-[16px] text-center leading-none">{{
                Number.isInteger(minPriceFlat) ? minPriceFlat.toLocaleString('ru') : "-" }} €</span>
              <span class="text-center text-[#8A8996] text-[13px] xxl:text-[11px] xl:text-[9px] lg:text-[13px]">{{
            ...
            <div class="flex flex-col justify-center sm:p-2">
              <span class="text-[18px] xxl:text-[15px] xl:text-[13px] lg:text-[16px] text-center leading-none">{{
                Number.isInteger(minSquareFlat) ? minSquareFlat : "-" }} - {{ Number.isInteger(maxSquareFlat) ?
    maxSquareFlat : "-" }}</span>
              <span class="text-center text-[#8A8996] text-[13px] xxl:text-[11px] xl:text-[9px] lg:text-[13px]">{{
          ...
          <div class="flex flex-col pb-14 xxl:pb-10 xl:pb-8" v-if="house.description">
            ...
            <p class="text-[#8A8996] text-base xxl:text-sm xl:text-xs pb-6 xxl:pb-5 xl:pb-4" v-else
              v-html="selectLanguage === 0 ? house.description !== null ? house.description.slice(0, 300) : house.description : selectLanguage === 1 ? house.description_en !== null ? house.description_en.slice(0, 300) : house.description_en : house.description_tr !== null ? house.description_tr.slice(0, 300) : house.description_tr + '...'">
            </p>
            <button class="flex gap-2 xxl:gap-1.5 xl:gap-1 items-center w-fit animation__arrow"
              @click="fullDescription = !fullDescription" v-if="house.description.length > 300">
              <span class="text-[#6435A5] font-medium text-sm xxl:text-xs xl:text-[10px] lg:text-[14px]">{{
          ...
          <div class="text-[18px] xxl:text-[15px] xl:text-[13px] lg:text-[16px] pb-14 xxl:pb-10 xl:pb-8"
          ...
          <div class="grid grid-cols-2 lg:grid-cols-1 gap-7 xxl:gap-5 xl:gap-4 pb-14 xxl:pb-10 xl:pb-8">
            <div class="border border-solid border-[#E5DFEE] p-7 xxl:p-5 xl:p-4 rounded-[12px]">
              <span class="font-medium text-[18px] xxl:text-[15px] xl:text-[13px] lg:text-[16px]">{{ language.rielt_1[45]
              }}</span>
              <div class="flex flex-col gap-5 xxl:gap-4 xl:gap-3 pt-6 xxl:pt-5 xl:pt-4">
                <div class="flex justify-between items-center" v-for="dop in dops">
                  <span :class="{ disableColor: dop.active !== 1 }"
                    class="text-base xxl:text-sm xl:text-xs lg:text-[15px]">{{ selectLanguage === 0 ? dop.name :
                      selectLanguage === 1 ? dop.name_en : dop.name_tr }}</span>
            ...
            <div class="border border-solid border-[#E5DFEE] p-7 xxl:p-5 xl:p-4 rounded-[12px]">
              <span class="font-medium text-[18px] xxl:text-[15px] xl:text-[13px] lg:text-[16px]">{{ language.rielt_1[24]
              }}</span>
              <div class="flex flex-col gap-5 xxl:gap-4 xl:gap-3 pt-6 xxl:pt-5 xl:pt-4">
                <div class="flex justify-between items-center" v-if="house.info.toSea !== null">
                  <span class="text-[#8A8996] text-base xxl:text-sm xl:text-xs xl:text-xs lg:text-[15px]">{{
                ...
                <div class="flex justify-between items-center" v-if="house.info.toShop !== null">
                  <span class="text-[#8A8996] text-base xxl:text-sm xl:text-xs lg:text-[15px]">{{ language.rielt_1[28]
                  }}</span>
                ...
                <div class="flex justify-between items-center" v-if="house.info.toChildrenSchool !== null">
                  <span class="text-[#8A8996] text-base xxl:text-sm xl:text-xs lg:text-[15px]">{{ language.rielt_1[29]
                  }}</span>
                ...
                <div class="flex justify-between items-center" v-if="house.info.toPark !== null">
                  <span class="text-[#8A8996] text-base xxl:text-sm xl:text-xs lg:text-[15px]">{{ language.rielt_1[33]
                  }}</span>
                ...
                <div class="flex justify-between items-center" v-if="house.info.toBus !== null">
                  <span class="text-[#8A8996] text-base xxl:text-sm xl:text-xs lg:text-[15px]">{{ language.rielt_1[34]
                  }}</span>
                ...
                <div class="flex justify-between items-center" v-if="house.info.toSchool !== null">
                  <span class="text-[#8A8996] text-base xxl:text-sm xl:text-xs lg:text-[15px]">{{ language.rielt_1[30]
                  }}</span>
        ...
        <app-decrition-object-sidebar class="w-full" v-if="!isWithClient" @open-all-news="openAllNews = true"
          :house="house" :language="language" />
      </div>
    </div>
    ...
    <div class="_container" v-if="user.role !== 1">
      <app-description-object-other-j-k :house="house" :slider="slider" :language="language"
        :selectLanguage="selectLanguage" :city_name="city_array" :area_name="area_array" />
    </div>
  </main>
